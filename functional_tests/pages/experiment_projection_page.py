...
class ProjectionExperimentPage(BasePage):
    ...
    def select_source_model(self,unique_id):
        source_models = self.driver.find_elements_by_css_selector("table.bccvl-datasetstable h1")
        for source_model in source_models:
            if source_model.text.find(unique_id) != -1:
                # Xpath .. retrieve's an element's parent.
                # We need the 3rd parent's checkbox
                source_model.find_element_by_xpath("..").find_element_by_xpath("..").find_element_by_xpath("..").find_element_by_css_selector("input").click()
                break
    ...
    def select_year(self,year):
        self._projection_tickboxes(year,"table.bccvl-yearstable h1")

    def select_emission_scenario(self,scenario):
        self._projection_tickboxes(scenario,"table.bccvl-emissionscenariostable h1")

    def select_climate_models(self,model):
        self._projection_tickboxes(model,"table.bccvl-climatemodelstable h1")
